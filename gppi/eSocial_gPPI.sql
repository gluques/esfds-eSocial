...
BEGIN    
    ...
    -------------------------------------------------------------------------------
    counter := 1;
    IF (llistaExpedient IS NULL) THEN
        numTotalExpedients := 1;
    ELSE
        numTotalExpedients := ARRAY_LENGTH(llistaExpedient, 1);        
    END IF;
    LOOP
        EXIT WHEN counter > numTotalExpedients;
        IF (llistaExpedient IS NOT NULL) THEN
            numeroExpedient := llistaExpedient[counter];
            prestacioId := NULL;
            dretId := NULL;
        END IF;
        -------------------------------------------------------------------------------
        ...
        RAISE INFO '';        
        RAISE INFO '----------------------------------------------------------------------------------------------------------------------------';	                
        IF (numeroExpedient IS NULL) THEN
            RAISE INFO ' Script eSocial gPPI - Fi';
        ELSE 
            RAISE INFO ' Script eSocial gPPI - Fi dades expedient %', numeroExpedient;
        END IF;
        RAISE INFO '----------------------------------------------------------------------------------------------------------------------------';	                
        RAISE INFO '';
        RAISE INFO '';        
